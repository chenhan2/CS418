...
class TriMesh{   
   ...
   */
    ...
    */
    ...
    {    
        //Your code here
        // Split fileText by lines
        let lines = fileText.split("\n");

        for (const line of lines) {
            // Split a line by whitespace
            let lineArray = line.split(/\b\s+(?!$)/);
            if (lineArray[0] == "v") {
                // Read a vertex
                this.vBuffer.push(parseFloat(lineArray[1]));
                this.vBuffer.push(parseFloat(lineArray[2]));
                this.vBuffer.push(parseFloat(lineArray[3]));
            } else if (lineArray[0] == "f") {
                // Read a face
                this.fBuffer.push(parseInt(lineArray[1]) - 1);
                this.fBuffer.push(parseInt(lineArray[2]) - 1);
                this.fBuffer.push(parseInt(lineArray[3]) - 1);
            } else if (lineArray[0] == "#") {
                // Read a comment
                continue;
            }
    ...
    */    
    ...
    canonicalTransform(){
      //Your code here
        ...
        let minXYZ=[0,0,0];
        let maxXYZ=[0,0,0];
        this.getAABB(minXYZ, maxXYZ);
        let translateVector = glMatrix.vec3.fromValues(- (minXYZ[0] + maxXYZ[0]) / 2, - (minXYZ[1] + maxXYZ[1]) / 2, - (minXYZ[2] + maxXYZ[2]) / 2)
        glMatrix.mat4.fromTranslation(T, translateVector);

        // Generate the S matrix
        let S = glMatrix.mat4.create();
        let L = 0;
        for (let i = 0; i < 3; i++) {
            L = Math.max(L, maxXYZ[i] - minXYZ[i]);
        }
        let scaleVector = glMatrix.vec3.fromValues(1/L, 1/L, 1/L);
        glMatrix.mat4.fromScaling(S, scaleVector);

        // Generate the modelMatrix
        let M = glMatrix.mat4.create();
        glMatrix.mat4.multiply(M, S, T);
        this.modelMatrix = M;
    }
